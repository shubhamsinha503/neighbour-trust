...
import { fetchConnectivity, NoDataError } from "@/lib/api";
...
export default async function ConnectivityPage({
  ...
  try {
    view = await fetchConnectivity(slug);
  } catch (error) {
    unavailable =
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-8">
      ...
      <h1 className="mb-5 mt-3 text-[23px] font-bold tracking-[-0.01em]">
        {categoryLabel(t, "infrastructure", "Connectivity")}{view ? ` — ${view.locality.name}` : ""}
      </h1>

      {view ? (
        <ConnectivityCard view={view} />
      ) : (
        <div className="rounded-[20px] border border-hairline bg-surface-1 p-5">
          <b className="text-[13px]">{t("sub.noConnectivity")}</b>
